...
@section('content')
    ...
    <div class="pt-3">
        <div class="d-flex justify-content-between align-items-center flex-wrap grid-margin mb-2 ">
            <div>
                <h4 class="mb-3 mb-md-0">Unit</h4>
            </div>
            <div class="d-flex align-items-center flex-wrap text-nowrap">
                @can('area_create')
                    <a href="{{ route('administrative.unit.create') }}"
    ...
    <script>
        $(document).ready(function() {
            $('#datatables').DataTable({
                ...
                processing: true,
                serverSide: true,
                ajax: "{{ route('administrative.unit.data') }}",
                columns: [{
                        data: 'id',
                        name: 'id'
                    },
        ...
        function deleteData(rowid) {
            let url = '{{ route('administrative.unit.destroy', ':id') }}';
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    axios.delete(url).then(res => {
                        ...
                        if (res.data == 'error') {
                            Swal.fire('This unit has multiple sub units', '', 'error')
                        }
                    });
                }
            })
