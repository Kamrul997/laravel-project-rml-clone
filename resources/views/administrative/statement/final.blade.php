        ...
        <div class="col-md-12 mt-4 grid-margin stretch-card">
            <div class="card">
                <div class="card-body card-wrapper">
                    <h3 class="title">Final Statement</h3>
...
@section('page-js')
    <script>
        ...
        $("#search-button").click(function() {
            ...
            $.post("/administrative/statement/final-statement-download", {
                order_id: order_id,
            }, function(data) {
                if (data.user_message == 'success') {
                    $('#download-body').removeClass('d-none');
                    $('#btn-download').attr('href', data.download_path);

                    // Split the string by '/'
                    var parts = data.download_path.split('/');
                    // Get the last element
                    var lastElement = parts[parts.length - 1];
                    $('.pdf-text').text(lastElement);
