    ...
    <div class="row">
        <div class="col-md-8 grid-margin stretch-card offset-md-2">
            <div class="card">
                <div class="card-body">
                    @if ($errors->has('file'))
                        <label id="file-error" class="error mt-2 text-danger"
                            for="file">{{ $errors->first('file') }}</label>
                    @endif
                    <form class="forms-sample" action="{{ route('store_target') }} " method="POST"
                        enctype="multipart/form-data">
                        @csrf
                        <div class="form-group {{ $errors->has('name') ? 'has-danger' : '' }}">
                            <label for="name">Select File</label>
                            <input required style="height: 35px;" type="file" id="file" name="file"
